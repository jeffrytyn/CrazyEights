...
/**
 * The ElevensBoard class represents the board in a game of Crazy Eights.
 */
public class CrazyEightsBoard extends Board {
    ...
    private List<Integer> findPairSum11(List<Integer> selectedCards) {
       super.SetCompHand(5);
        ...
        for (int sk1 = 0; sk1 < selectedCards.size(); sk1++) {
            ...
            if (cardAt(k1).pointValue() == cardAt(super.GetCompHand()).pointValue()) {
                foundIndexes.add(new Integer(k1));
                //foundIndexes.add(new Integer(CompHand));
                return foundIndexes;
            }
            if (cardAt(k1).suit() == cardAt(super.GetCompHand()).suit()) {
                foundIndexes.add(new Integer(k1));
                //foundIndexes.add(new Integer(CompHand));
                return foundIndexes;
            }
    ...
    private boolean playPairSum11IfPossible() {
        ...
        if (cardsToReplace.size() > 0) {
            ...
            if (I_AM_DEBUGGING) {
                System.out.println("matched house or value\n");
            }
            return true;
        } else {
            return false;
